...
printf "${white}"
	...
	printf "${yellow}"
	curl -X POST https://www.heliumstats.online/code-red/status.php -H 'Content-Type: application/json-rpc' -d '{"hostname":"'"$HNAME"'","message": "Upgrading Server Packages..."}' && echo -e " "
	echo -e "------------------------------------------------- " | tee -a "$LOGFILE"
